...
def run_drive_motors(forward,strafe,turn):
    ...
    if reverseDriveToggle: 
        forward*=-1
        strafe*=-1
    turn = gps_drive_assist(forward, strafe, turn)
...
def newgps_heading_control(turn):
    ...
    #runs the heading correction after it has the heading, which also means it only kicks in for longer drives
        ...
        turn,prev_head_error,total_head_error=PID(0,-heading_error,1.5,0.005,0.3 ,prev_head_error,total_head_error)  #THIS IS PID! please tune the values, the .25 is just from what i used before. 
        if turn>50: turn=50
        elif turn < -50:
            turn = -50
